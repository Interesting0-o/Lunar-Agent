...
> 2026-06-20 | 192 用例全部通过 | 153s | 三项 P0 修复：social_battery 结构性修复 + β 逐维度解耦 + 表面层 stress 去放大
...
**防御剖面 sigmoid 缩放 + β 调制公式修复（06-18）。** sigmoid 从 `_sigmoid(x − 0.48)` 改为 `_sigmoid((x − 0.35) × 5.0)`，β 公式从 `0.10 + hyper×0.14 − deact×0.10` 改为 `0.05 + hyper×0.35 − deact×0.15`。β 有效变动从 0.042 提升到 0.289（×6.9），4 个此前失效的特质恢复有效影响。

**三项 P0 修复（06-20）：**

1. **social_battery 结构性修复。** `INPUT_INFLUENCE_B` 增加 `ST_VALIDATION→+0.15`、`ST_CLOSENESS→−0.10 改为 +0.08`；新增能量耦合 `I_ENERGY→I_SOCIAL_BATTERY: +0.08`；自阻尼从向 0 回归改为向 `DECAY_TARGETS[I_SOCIAL_BATTERY]=0.20` 回归。social_battery 从"只降不升"变为双向可调。

2. **β 从全局标量改为逐刺激维度向量。** 删除 `hyper.mean()` 全局均值，改为 `beta_stim = 0.05 + hyper*0.35 − deact*0.15` 生成 `(7,)` 逐刺激向量。每个刺激有自己的接受率，保留防御剖面对具体刺激类型的选择性。

3. **表面层 stress 三维重叠放大修复。** stress 出口从 warmth(−0.20)+sharpness(+0.20)+softness(−0.30) 三路叠加改为 warmth(−0.15)+sharpness(+0.15)+restraint(+0.20)；fatigue 系数从 0.30 降至 0.15 解除等高对冲；新增 `validation→enthusiasm: +0.15` 正刺激入口。
...
| `social_battery` 单轮最大变化 | 0.0710 | 🟢 | 双向响应，validation/亲密充电、冲突消耗 |
...
| β 结构 | `0.05 + **hyper.mean()**×0.35 − **deact.mean()**×0.15`（全局标量） | `0.05 + **hyper[s]**×0.35 − **deact[s]**×0.15`（逐维度向量） |
| β 调制 | `beta × delta_stimulus`（全局缩放） | `beta_stim * inner_stimuli`（逐元素乘后再进 B 矩阵） |

**修复后 β 统计（n=50,000 随机参数，06-20 逐维版本）**：

| 指标 | 修复前 (06-18 标量) | 修复后 (06-20 逐维) |
|------|:------:|:------:|
| 标量 β 范围 | [0.096, 0.138] | **[0.010, 0.299]** |
| 标量 β 有效变动 | 0.042 | **0.289** (×6.9) |
| 标量 β 变异系数 | 0.055 | **0.480** (×8.7) |
| 低防御 β | ≈0.10 | **0.038** |
| 高防御 β | ≈0.14 | **0.206** |
| **逐维 β 内标准差** | N/A（无逐维概念） | μ=**0.0417** σ=0.0187 |
| **逐维 β 退化率 (σ<0.005)** | N/A | **1.3%** ✅ |

防御剖面现在真正影响响应速率，且保留刺激特异性：同一剖面下 7 种刺激的 β 平均标准差 0.042，退化率仅 1.3%（即 98.7% 的参数空间中各刺激的接受率有显著区分）。高焦虑型角色 β↑+高特异性，高回避型角色 β↓+低特异性。
...
|------|:-----:|------|:-----:|:----:|
| stress | 0.0187 | warmth | 0.0821 | 4.4x |
| stress | 0.0187 | sharpness | 0.0805 | 4.3x |
| stress | 0.0187 | restraint | 0.0552 | 3.0x（新增出口） |
| loneliness | 0.0251 | softness | 0.0636 | 2.5x（由 emotional_safety 主导） |
| insecurity | 0.0218 | vulnerability | 0.0479 | 2.2x |
| insecurity | 0.0218 | restraint | 0.0552 | 2.5x |
| energy | 0.0151 | expressiveness | 0.0069 | 0.5x（压缩） |
| energy | 0.0151 | enthusiasm | 0.0415 | 2.7x |

**修复效果**：
- stress 不再直接压 softness，softness 改由 emotional_safety(2.5x) 驱动，单独可控
- stress 新增 restraint 出口（3.0x），将部分"负面放大"转为中性克制表达
- enthusiasm 新增 validation 正刺激入口，正面表达不再被 fatigue 等高对冲
- **stress 三路负面叠加已消除**，softness 不再成为 stress 的间接伤害通道
...
- 最大维度漂移: **0.0103**（远低于 0.05 阈值）
- 平均维度漂移: 0.0091
- 无边界违规
- 结论：零均值矩阵噪声未引入系统性偏差

#### 场景 3: 倒置映射（温暖→冲突），1000 轮 ✅ 零异常

| 维度 | 旧尾均值 | 新均值 | 改善 |
|------|:-----:|:-----:|:----:|
| social_battery (A) | +0.045 🔴 | **+0.678** | **从几乎归零到健康充电** |
| familiarity (A) | -0.268 🔴 | **+0.292** | 从负转正 |
| dependency (A) | -0.236 🔴 | **+0.140** | 从负转正 |

social_battery 的改善最为显著：B 矩阵 closeness+0.08 意味着倒置映射下角色仍能从交互中获得正刺激（B 映射逆温→亲密度升高→社交充电），不再结构性漏电。

#### 场景 4: 噪声感知+周期性扰动，1200 轮 ✅ **零异常（原 14 🔴 → 0）**

**修复后统计**（Agent A）：

| 维度 | 均值 | 范围 | 判定 |
|------|:----:|:----:|:----:|
| stress | +1.074 | [-0.500, +1.097] | ⚠️ 发散但稳定 |
| irritation | +1.058 | [-0.774, +1.094] | ⚠️ 发散但稳定 |
| mental_fatigue | +1.058 | [-0.675, +1.093] | ⚠️ 发散但稳定 |
| **social_battery** | **-1.019** | **[-1.072, +0.201]** | ⚠️ 边界钳位 |
| familiarity | +0.509 | [-0.563, +0.804] | ⚠️ 发散 |
| dependency | +0.762 | [-0.593, +1.031] | ⚠️ 发散 |
| trust | -0.999 | [-1.050, -0.312] | ⚠️ 发散 |
| emotional_safety | -1.014 | [-1.050, -0.431] | ⚠️ 发散 |

**此前 14 个 🔴 全部消除**。social_battery 触及负边界（-1.019）但系统稳定——无 NaN、无越界、无振荡。边界行为由 soft_clamp 的过渡区（允许 ±1.1）平滑处理。

**观察**：能量耦合 `I_ENERGY × 0.08 → I_SOCIAL_BATTERY` 在敌对场景中变成副作用——当能量极低（-0.92）时耦合转为 -0.074/轮的额外消耗。这是合理行为（疲惫时社交恢复更慢），但在极端对抗场景中叠加了 drain。普通对话中能量为正，耦合恰好是充电。
...
| 倒置映射（温暖→冲突） | 1000 | **0（原4）** | ✅ 修复——social_battery 从 +0.045 升至 +0.678 |
| 噪声+周期性扰动 | 1200 | **0（原14）** | ✅ 修复——无边界违规，social_battery 边界钳位但不发散 |
...
| 🟢 P2 | 6 | 2.8 vulnerability 触底5%, 2.10 single-stimulus 影响不均衡, 2.11 expressiveness 压缩(0.5x)仍存（enthusiasm 已修复+validation入口）, 2.5 剖面可达极值尚可优化, 2.6 少数特质敏感度仍偏低, 2.15 内部态渐近残余（受 k=0.05 限制，非 bug 但需文档化） |
...
| **2.11 表面层 stress 4.5x 放大** | **stress 出口分散（warmth-0.15/sharpness+0.15/restraint+0.20），softness 改由 emotional_safety 主导，fatigue 系数 0.30→0.15 解除对冲** |
| **2.12 social_battery 跌负** | **B 矩阵增正输入、增能量耦合、自阻尼向 0.20 而非 0，social_battery 从只降不升变为双向可调** |
| **2.4 β 均值掩盖刺激特异性** | **β 从全局标量 hyper.mean() 改为 (7,) 逐维度向量，保留防御剖面对具体刺激类型的响应选择性** |
...
| **报告文档化** | **06-19** | 新增 2.15 时间衰减非对称性、Layer 9 测试层级、运行命令更新 |
| **social_battery 结构性修复** | **06-20** | B 矩阵增正输入、能量耦合、自阻尼向 0.20。P0 关闭 |
| **β 逐维度解耦** | **06-20** | β 从 hyper.mean() 全局标量改为 per-stimulus 向量。保留防御特异性 |
| **表面层 stress 去放大** | **06-20** | stress 三路出口分散 + fatigue 对冲解除 + enthusiasm 正入口。P0 关闭 |
| **异常测试修复** | **06-20** | 修正 test_beta_vs_alpha_balance 硬编码、test_beta_range_after_fix 补充 per-stimulus 检查、test_surface_amplification 自相减死代码 |
| **报告更新** | **06-20** | 更新 2.1/2.4/2.11/2.12/六/已关闭/本版记录 |
