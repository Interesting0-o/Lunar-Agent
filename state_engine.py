...
# ============================================================
# ④ Gate Control —— 压抑/脆弱/依恋 门控
# ============================================================
#
# 这是角色"潜意识"的核心——Gate 由人格特质决定：
#   - suppression_gate:  压抑程度，由 pride/开放/稳定性 决定
#   - vulnerability_gate: 示弱意愿，pride↓ + emotional_openness↑
#   - attachment_gate:   依恋敏感度
#   - leakage_gate:      不再依赖隐藏状态累积，设为 0
#
# 压抑/泄漏的"里表情"效应已合并到 Surface Projection 中处理。
...
def compute_gates(
    ...
    # Suppression Gate: 高自尊 + 低情绪开放 + 低稳定 → 压抑
    gates[G_SUPPRESSION] = np.clip(
        traits[T_PRIDE] * 0.4
    ...
    # Leakage Gate: 过去依赖隐藏状态累积，移除后恒为 0
    gates[G_LEAKAGE] = 0.0
...
# 表面表达 = 内部状态 + 关系状态 + 特质修饰 的"可见部分"。
#
# 注意：压抑/隐藏等"里表情"效应已通过 Gate Control
#（决定多少刺激进入内部状态）和特质修饰直接嵌入投影函数中，
# 不再需要单独的隐藏状态层。
...
def initialize_all(traits: np.ndarray) -> dict:
    ...
    surface = project_surface(internal, relationship, traits)
...
def update_all(
    ...
    surface = project_surface(new_internal, new_relationship, traits)
